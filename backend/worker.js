...
function getCorsHeaders(request, env) {
  ...
  const allowed = (env.FRONTEND_ORIGIN || 'http://localhost:5173')
    .split(',')
    .map((o) => o.trim())
    .filter(Boolean);
...
async function sendEmail(env, { to, replyTo, subject, text, html }) {
  // Method 1: If RESEND_API_KEY is configured, use HTTP REST API
  if (env.RESEND_API_KEY) {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: env.FROM_EMAIL || `Viet Entry Travel <${env.SMTP_USER || 'onboarding@resend.dev'}>`,
        to: [toEmail],
  ...
  // Method 2: SMTP via WorkerMailer (uses cloudflare:sockets)
  const port = Number(env.SMTP_PORT) || 465;
  const isSecure = env.SMTP_SECURE === 'true' || port === 465;
  const rawHost = env.SMTP_HOST || 'pro218.emailserver.vn';
  ...
  await mailer.send({
    from: { name: 'Viet Entry Travel', email: env.SMTP_USER },
    to,
...
export default {
  async fetch(request, env, _ctx) {
    ...
    if (url.pathname === '/health' || url.pathname === '/') {
      return jsonResponse({ ok: true, service: 'viet-entry-travel-worker' }, 200, corsHeaders);
    }
    ...
    if (url.pathname === '/api/quote-requests' && request.method === 'POST') {
      ...
      if (!name || typeof name !== 'string' || !name.trim()) {
        return jsonResponse({ ok: false, error: 'Please enter your name.' }, 400, corsHeaders);
      }
      if (!isValidEmail(email)) {
        return jsonResponse({ ok: false, error: 'Please enter a valid email address.' }, 400, corsHeaders);
      }
  ...
  <div style="width:100%;background-color:#f3f4f6;padding:32px 12px;">
    <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width:600px;margin:0 auto;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.07);border:1px solid #e5e7eb;">
      ...
      try {
        ...
        await sendEmail(env, {
          to: { name: 'Admin', email: env.OWNER_EMAIL || 'info@vietentrytravel.com' },
          replyTo: { name, email },
        ...
        // 2. Send confirmation auto-reply to customer
        try {
          await sendEmail(env, {
            to: { name, email },
            replyTo: { name: 'Viet Entry Travel', email: env.OWNER_EMAIL || 'info@vietentrytravel.com' },
            subject: `Your Vietnam Journey Awaits [${referenceCode}] — Viet Entry Travel`,
            text: customerTextBody,
            html: customerHtmlBody,
          });
        } catch (custErr) {
          console.error('[worker] Failed to send customer confirmation:', custErr);
        }

        return jsonResponse({ ok: true, referenceCode }, 200, corsHeaders);
      } catch (err) {
        console.error('[worker] Email sending error:', err);
        return jsonResponse(
          {
            ok: false,
            error: 'We could not send your request right now. Please try again shortly or contact us via WhatsApp.',
          },
          500,
          corsHeaders
        );
