...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({ ok: true, service: 'quote-requests-api' });
  }
...
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;color:#1e293b;">
  ...
  const smtpHost = process.env.SMTP_HOST || 'pro218.emailserver.vn';
  const smtpPort = Number(process.env.SMTP_PORT) || 465;
  const smtpSecure = process.env.SMTP_SECURE ? process.env.SMTP_SECURE === 'true' : smtpPort === 465;
  const smtpUser = process.env.SMTP_USER || 'info@vietentrytravel.com';
  const smtpPass = process.env.SMTP_PASS || 'Canmoc1ti2026@';
  const ownerEmail = process.env.OWNER_EMAIL || 'info@vietentrytravel.com';
  ...
  try {
    await transporter.sendMail({
      from: `"Viet Entry Travel Website" <${smtpUser}>`,
      to: ownerEmail,
      replyTo: `"${name}" <${email}>`,
    ...
    try {
      await transporter.sendMail({
        from: `"Viet Entry Travel" <${smtpUser}>`,
        to: `"${name}" <${email}>`,
        replyTo: ownerEmail,
      ...
      console.error('Customer email confirmation error:', custErr);
    }

    return res.status(200).json({ ok: true, referenceCode });
  } catch (err) {
